...
export const createTrade = async ({email, gameId, itemsGive, itemsReceive, paymentMethodId, promocode, price}: CreateTradeI) => {
    let sendItemsGive = itemsGive.map((item) => item.id)
    let sendItemsReceive = itemsReceive.map((item) => item.id)
    const resp = await axios.post<TradeInfoI>(`${process.env.api}/create_trade`, {
        email: email,
        gameId: gameId,
